...
# Local version for GTX 3060 on Windows (no Flash Attention 3, no Triton, no NCCL)
# Run with: TORCHDYNAMO_DISABLE=1 RUN_ID=smoke ITERATIONS=200 TRAIN_BATCH_TOKENS=8192 VAL_LOSS_EVERY=0 python train_gpt_local.py
import collections,copy,glob,io,lzma,math,os
from pathlib import Path
import random,re,subprocess,sys,time,uuid,numpy as np,sentencepiece as spm,torch,torch.distributed as dist,torch.nn.functional as F
from torch.nn.parallel import DistributedDataParallel as DDP
from torch import Tensor,nn
def _maybe_compile(fn_or_model,**kwargs):
	if os.environ.get('TORCHDYNAMO_DISABLE','0')=='1':return fn_or_model
	return torch.compile(fn_or_model,**kwargs)
...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x):
		...
		# Transpose from (B,T,H,D) to (B,H,T,D) for native SDPA
		q_sdpa=q.transpose(1,2);k_sdpa=k.transpose(1,2);v_sdpa=v.transpose(1,2)
		# Expand KV heads for GQA: (B,Hkv,T,D) -> (B,H,T,D)
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=_maybe_compile(base_model,dynamic=False,fullgraph=True)
...
def train_and_eval(h,device):
	random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device)
	train_shard_count=len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))
	log(f"train_shards: {train_shard_count}");log(f"val_tokens: {val_data.val_tokens.numel()-1}");base_model,compiled_model=train_model(h,device,val_data);torch._dynamo.reset();timed_eval('pre-quantization post-ema',eval_val,h,device,val_data,compiled_model);serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
	if h.distributed:dist.barrier()
	eval_model=deserialize(h,device)
	if h.num_loops>0:eval_model.looping_active=True
	compiled_model=_maybe_compile(eval_model,dynamic=False,fullgraph=True);timed_eval('quantized',eval_val,h,device,val_data,compiled_model)
...
def main():
	...
	torch.backends.cuda.matmul.allow_tf32=True;torch.backends.cudnn.allow_tf32=True;torch.set_float32_matmul_precision('high');from torch.backends.cuda import enable_cudnn_sdp,enable_flash_sdp,enable_math_sdp,enable_mem_efficient_sdp;enable_cudnn_sdp(False);enable_flash_sdp(True);enable_mem_efficient_sdp(False);enable_math_sdp(True);torch._dynamo.config.optimize_ddp=False;h=Hyperparameters();set_logging_hparams(h)
	if h.is_main_process:
